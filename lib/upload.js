...
try {
  ...
  // remove the source directory to make the slug smaller
  var RM_AWS_STATIC_SOURCE_DIRECTORY = getEnvVariable('RM_AWS_STATIC_SOURCE_DIRECTORY') == 'true';
  // remove node_modules folder
  var RM_NODE_MODULES = getEnvVariable('RM_NODE_MODULES') == 'true';
...
var STATIC_PATH = path.join(AWS_STATIC_PREFIX, getEnvVariable('HEROKU_APP_NAME'), 'v' + (Number(getEnvVariable('HEROKU_RELEASE_VERSION').slice(1)) + 1), uuidv4());
...
glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**/*.*', {}, function(error, files) {
    ...
    async.eachLimit(files, 16, function(file, callback) {
      ...
      function onUploadComplete(error) {
        ...
        if (RM_AWS_STATIC_SOURCE_DIRECTORY) {
          console.log('Deleting AWS_STATIC_SOURCE_DIRECTORY');
          execSync(`cp ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/assets/.sprockets-manifest*.json ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/../tmp`);
          execSync(`mkdir -p ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/../tmp/packs`);
          execSync(`cp ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/packs/manifest.json ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/../tmp/packs/manifest.json`);
          
          execSync(`rm -rf ${PUBLIC_ASSETS_SOURCE_DIRECTORY}`);

          execSync(`mkdir -p ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/assets`);
          execSync(`mkdir -p ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/packs`);
          execSync(`cp ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/../tmp/.sprockets-manifest*.json ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/assets`);
          execSync(`cp ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/../tmp/packs/manifest.json ${PUBLIC_ASSETS_SOURCE_DIRECTORY}/packs/manifest.json`);
        }

        if (RM_NODE_MODULES) {
          console.log('Deleting node_modules');
          execSync(`rm -rf ${path.join(BUILD_DIR, 'node_modules')}`);
        }
